...
import {getBookByAuthor, getBookByType, getBooks} from "../Service/BookService.js";
...
function withMyHook(Component) {
    return function WrappedComponent(props) {
        ...
        return <Component {...props} myHookValue2={messageApi} myHookValue3={contextHolder} />;
    }
}
// const data1 = [
//     {
...
export class BookList extends React.Component
{
    ...
    onSearch = (values) =>
    {
        ...
        getBookByAuthor({"search":null, name: values}, callback);
    ...
    onSearch2 = (values) =>
    {
        const callback =  (data) => {
            if (data.name != undefined)
                message.info("the author is " + data.name );
            else
                message.info("no such author!" );
    ...
    {
        const callback =  (data) => {
            this.setState({shownbook: data});
        };
        getBookByType({"search":null, type: values}, callback);
    ...
    render ()
    {
        ...
        <div className={"mySearchBar"}>
            <Search placeholder = "search the author" style = {{width: 500}} onSearch = {this.onSearch2} enterButton = "Search"></Search>
            <Search placeholder = "input search text" style = {{width: 500}} onSearch = {this.onSearch} enterButton = "Search"></Search>
            <Search placeholder = "search similar type of books" style = {{width: 500}} onSearch = {this.onSearch3} enterButton = "Search"></Search>
        </div>
            <List grid = {{
                gutter: 5,
                column: 4,
            ...
            dataSource = {this.state.shownbook}
            renderItem = {item => (
                    <List.Item>
                        <Book  info = {item} />
                    </List.Item>
